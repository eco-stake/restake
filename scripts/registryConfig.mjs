...
const registryConfig = (name) => {
  const generateConfig = (chain, tokens, existingConfig) => {
    ...
    const fees =
      (chain.fees && chain.fees.fee_tokens && chain.fees.fee_tokens[0]) || {};
    let gasPrice;
    ...
    return {
      ...
      rpcUrl: _.uniq(
        chain.apis.rpc.map((el) => el.address.replace(/\/$/, "")),
  ...
  const testGrants = (restUrl) => {
    return axios
      .get(restUrl + "/cosmos/authz/v1beta1/grants")
      .then((res) => res.data)
      .then((data) => data)
      .catch((error) => {
...
if (config.restUrl.length) {
  const restClient = await RestClient(
    config.chainId,
    config.rpcUrl,
    config.restUrl
  );
  if (!restClient.restUrl)
    console.log("No API responses. Unable to check authz status");
