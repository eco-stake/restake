...
const SigningClient = async (rpcUrl, chainId, defaultGasPrice, signer, key) => {
  ...
  const client =
    rpcUrl && (await SigningStargateClient.connectWithSigner(rpcUrl, signer));
  ...
  const getIsNanoLedger = () => key.isNanoLedger;
  ...
  const signAndBroadcastWithoutBalanceCheck = (
    address,
    msgs,
    gas,
    memo,
  ...
  function findAvailableUrl(urls) {
    return findAsync(urls, (url) =>
      axios
        .get(`${url}/status?`, { timeout: 1000 })
        .then((res) => res.data)
        .then((data) => data.result.node_info.network === chainId)
        .catch((error) => false)
    );
  }
