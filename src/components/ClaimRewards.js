...
function ClaimRewards(props) {
  ...
  async function claim() {
    ...
    const totalReward = validatorRewards.reduce(
      (sum, validatorReward) => add(sum, bignumber(validatorReward.reward)),
      0,
    );
    const adjustedValidatorRewards = validatorRewards.map((validatorReward) => {
      const shareOfFee = multiply(
        divide(bignumber(validatorReward.reward), totalReward),
        feeAmount,
      ); // To take a proportional amount from each validator relative to total reward
      return {
    ...
    if (
      !props.commission &&
      (adjustedValidatorRewards.length < 1 ||
        adjustedValidatorRewards.some(
          (validatorReward) => validatorReward.reward <= 0,
        ))
    ) {
      props.setLoading(false);
    ...
    signAndBroadcast(wallet.address, messages, gas).then(
      (result) => {
