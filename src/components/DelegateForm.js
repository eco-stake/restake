...
class DelegateForm extends React.Component {
  ...
  async handleSubmit(event) {
    ...
    client.signAndBroadcast(wallet.address, messages, gas, memo).then(
      (result) => {
  ...
  buildMessages(amount) {
    ...
    if (this.props.redelegate) {
      ...
      typeUrl =
        ...
        (this.props.undelegate ? "Undelegate" : "Delegate");
      value = {
        delegatorAddress: address,
  ...
  hasPermission() {
    const permission = this.props.redelegate
      ? "BeginRedelegate"
      : this.props.undelegate
      ? "Undelegate"
      : "Delegate";
  ...
  async setAvailableAmount() {
    ...
    const messages = this.buildMessages(
      multiply(this.props.availableBalance.amount, 0.95),
    );
    const decimals = pow(10, this.props.network.decimals);
    ...
    this.props.signingClient.simulate(this.props.wallet.address, messages).then(
      (gas) => {
        const gasPrice = this.props.signingClient.getFee(gas).amount[0].amount;
        const saveTxFeeNum = 10;
        const amount = divide(
          subtract(balance, multiply(gasPrice, saveTxFeeNum)),
          decimals,
        );
              ...
              <div className="mb-3">
                ...
                {this.props.availableBalance && (
                  <div className="form-text text-end">
                    <span
                      role="button"
                      onClick={() => this.setAvailableAmount()}
                    >
                      Available:{" "}
                      <Coins
