...
function ValidatorServices(props) {
  ...
  if (validator.services?.staking_rewards) {
    ...
    if (theme === "dark")
      icon = verified
        ? StakingRewardsVerifiedWhiteIcon
        : StakingRewardsWhiteIcon;
    services.push({
      key: "stakingrewards",
      tooltip: (
        <span>
  ...
  const mintscan = network.chain.explorers?.find(
    (el) => el.kind === "mintscan",
  );
  if (mintscan) {
    services.push({
  ...
  const guru = network.chain.explorers?.find(
    (el) => el.kind === "explorers.guru",
  );
  if (guru) {
    services.push({
      key: "nodesguru",
  ...
  const showServices = services.filter(
    (el) => !props.show || props.show.includes(el.key),
  );
