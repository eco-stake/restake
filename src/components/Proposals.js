...
function Proposals(props) {
  ...
  function filteredProposals(proposals, filter) {
    ...
    const searcher = new FuzzySearch(searchResults, ["content.title"], {
      sort: true,
    });
    ...
    return (
      <tr key={proposalId}>
        ...
        <td className="d-none d-md-table-cell text-center">
          <ProposalProgress proposal={proposal} tally={tallies[proposalId]} />
        </td>
        <td>
    ...
    <>
          ...
          >
            <Nav.Item>
              <Nav.Link
                eventKey="voting"
                disabled={
                  filteredProposals(proposals, { ...filter, group: "voting" })
                    .length < 1
                }
              >
                Voting Period
      ...
      {results.length > 0 && (
        <Table className="align-middle table-striped">
          ...
          <tbody>{results.map((item) => renderProposal(item))}</tbody>
        </Table>
      )}
      {results.length < 1 && (
