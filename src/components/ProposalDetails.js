...
function ProposalDetails(props) {
  ...
  useEffect(() => {
    if (
      props.address !== props.wallet?.address &&
      props.granters.includes(props.address)
    ) {
  ...
  useEffect(() => {
    if (granter) {
      props.queryClient
        .getProposalVote(proposal_id, granter)
        .then((result) => {
  ...
  return (
    <>
              ...
              {!proposal.isDeposit && (
                <>
                  <tr>
                    ...
                    <td>
                      <Moment format="LLL">{proposal.submit_time}</Moment>
                    </td>
                  </tr>
                  <tr>
                    <td scope="row">Voting end time</td>
                    <td>
                      <Moment format="LLL">{proposal.voting_end_time}</Moment>
                    </td>
                  </tr>
                </>
              )}
              {proposal.isDeposit && (
                <>
                  <tr>
                    ...
                    <td>
                      <Moment format="LLL">{proposal.deposit_end_time}</Moment>
                    </td>
                  </tr>
                  <tr>
                ...
                >
                  ...
                  <optgroup label="Authz Grants">
                    {props.granters.map((granterAddress) => {
                      const favourite = props.favouriteAddresses.find(
                        (el) => el.address === granterAddress,
                      );
                      return (
                        <option key={granterAddress} value={granterAddress}>
                          {favourite?.label || granterAddress}
                        </option>
                      );
                    })}
                  </optgroup>
      ...
      <div className="mb-4 mt-2">
        <ProposalProgress proposal={proposal} tally={tally} height={25} />
      </div>
      <div className="row mt-3">
        <div className="col">
          <h5 className="mb-3">{title}</h5>
          <p style={{ whiteSpace: "pre-wrap" }}>{fixDescription}</p>
        </div>
      </div>
    </>
  );
