...
class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      favouriteAddresses: favouriteAddressJson
        ? JSON.parse(favouriteAddressJson)
        : {},
    };
  ...
  getSignerProvider(providerKey) {
    return (
      providerKey && this.signerProviders().find((el) => el.key === providerKey)
    );
  }
  ...
  async connect(providerKey, manual) {
    ...
    try {
      ...
      try {
        ...
        return this.setState({
          error: `Failed to connect to ${signerProvider?.label || "signer"}: ${
            e.message
          }`,
        });
      }
  ...
  toggleFavouriteAddress(address, label) {
    ...
    this.updateFavouriteAddresses({
      ...this.state.favouriteAddresses,
      [this.props.network.path]: newFavourites,
    });
  }
  ...
  currentFavouriteAddress() {
    return this.favouriteAddresses().find(
      (el) => el.address === this.state.address,
    );
  }

  otherFavouriteAddresses() {
    return this.favouriteAddresses().filter(
      (el) => el.address !== this.state.wallet?.address,
    );
  }

  viewingWallet() {
    return (
      this.state.address && this.state.address === this.state.wallet?.address
    );
  }
  ...
  async getGrants() {
    ...
    addresses = addresses.concat(
      favourites
        .filter((el) => !addresses.includes(el.address))
        .map((el) => el.address),
    );

    granterGrants = await this.getGrantsIndividually(
      addresses.map((el) => {
    ...
    this.setState((state) => {
      return {
        grantQuerySupport,
        grants: { ...state.grants, granter: granterGrants },
      };
    });
    granteeGrants = await this.getGrantsIndividually(
      favourites.map((el) => {
    ...
    this.setState((state) => {
      return {
        grantQuerySupport,
        grants: { ...state.grants, grantee: granteeGrants },
      };
    });
  }
  ...
  onGrant(grantee, grant) {
    const filterGrant = (el) => {
      ...
      if (
        el.authorization["@type"] === grant.authorization["@type"] &&
        el.authorization.msg === grant.authorization.msg
      ) {
        return false;
      }
      return true;
  ...
  onRevoke(grantee, msgTypes) {
    const filterGrant = (el) => {
      ...
      if (
        ...
        msgTypes.includes(el.authorization.msg)
      )
        return false;
      return true;
    };
  ...
  themeIcon() {
    ...
    if (themeChoice === "auto") {
      ...
      icon =
        themeChoice === "dark" ? (
          <DropletFill {...iconProps} />
        ) : (
          <Droplet {...iconProps} />
        );
      switchTo =
        themeDefault !== theme ? "auto" : theme === "dark" ? "light" : "dark";
    }
  ...
  introText() {
    switch (this.props.active) {
      ...
      case "governance":
        return (
          <span>
            REStake let's you vote on behalf of your other{" "}
            {this.props.network && (
              <strong
      ...
      case "grants":
        return (
          <span>
            ...
            Authz grants in one place
          </span>
        );
    }
    return (
      <span>
        REStake allows validators to{" "}
        <strong
    ...
    return (
      <Container>
        <header className="">
          <div className="d-flex justify-content-between align-items-center py-3 border-bottom">
            <div className="logo d-flex align-items-end text-reset text-decoration-none">
              <span
                onClick={() => this.props.setActive("networks")}
                role="button"
              ...
              >
                {this.props.theme === "light" ? (
                  ...
                  <img
                    src={LogoWhite}
                    srcSet={`${LogoWhite2x} 2x, ${LogoWhite3x} 3x`}
                    alt="REStake"
                  />
                )}
              </span>
          ...
          <div className="d-flex justify-content-between border-bottom">
            <Navbar
              className={`navbar navbar-expand ${
                this.props.theme === "dark" ? "navbar-dark" : "navbar-light"
              }`}
            >
              <div className="justify-content-center">
                <Nav
                  activeKey={this.props.active}
                  onSelect={(e) => this.props.setActive(e)}
                >
                    ...
                    <>
                      <div className="nav-item px-2 border-end">
                        <Nav.Link eventKey="delegations">
                          ...
                          <span className="d-none d-sm-inline"> Stake</span>
                        </Nav.Link>
                      </div>
                      <div className="nav-item px-2 border-end">
                        <Nav.Link eventKey="governance">
                          <Inboxes className="mb-1 me-1" />
                          <span className="d-none d-sm-inline"> Govern</span>
                        </Nav.Link>
                      </div>
                      {this.state.address &&
                        this.props.network.authzSupport && (
                          <div className="nav-item ps-2">
                            <Nav.Link eventKey="grants">
                              <Magic className="mb-1 me-1" />
                              <span className="d-none d-sm-inline"> Grant</span>
                            </Nav.Link>
                          </div>
                        )}
                    </>
              ...
              <div className="justify-content-center">
                <ul className="navbar-nav">
                  {this.props.network && (
                    <>
                      {(this.state.wallet ||
                        this.favouriteAddresses().length > 0) && (
                        <li className="nav-item pe-3 border-end d-flex align-items-center">
                          ...
                          this.state.wallet ? (
                            <span
                              ...
                              className="small d-none d-lg-inline ms-2"
                            >
                              {this.state.wallet.name ||
                            ...
                            <select
                              className="form-select form-select-sm d-none d-lg-block ms-2"
                              aria-label="Address"
                              value={this.state.address || ""}
                              onChange={(e) =>
                            ...
                            >
                              {this.state.wallet ? (
                                <optgroup
                                  label={this.state.signerProvider.label}
                                >
                                  <option value={this.state.wallet.address}>
                                    {this.state.wallet.name ||
                                      this.state.wallet.address}
                                  </option>
                                </optgroup>
                              ...
                              <optgroup label="Saved">
                                {this.otherFavouriteAddresses().map(
                                  ({ address, label }) => {
                                    return (
                        ...
                        <li className="nav-item px-3 border-end align-items-center d-none d-md-flex">
                          {this.state.balance ? (
                            ...
                            <Spinner
                              animation="border"
                              role="status"
                              className="spinner-border-sm text-secondary"
                            >
                          ...
                          <Dropdown.Menu>
                            {this.state.address && (
                              <div className="d-block d-md-none">
                                <Dropdown.Header className="text-truncate">
                                  {this.addressName()}
                                </Dropdown.Header>
                            ...
                            {this.state.wallet ? (
                              <>
                                <Dropdown.Item
                                  as="button"
                                  disabled={
                                    !this.state.wallet?.hasPermission(
                                      this.state.address,
                                      "Send",
                                    )
                                  }
                              ...
                              this.signerProviders().map((provider) => {
                                return (
                                  <Dropdown.Item
                                    ...
                                    key={provider.key}
                                    onClick={() =>
                                      this.connect(provider.key, true)
                                    }
                            ...
                            {this.state.address && (
                              <>
                                ...
                                >
                                  Disconnect {this.state.signerProvider?.label}
                                </Dropdown.Item>
                              </>
                            )}
                          </Dropdown.Menu>
        ...
        <div className="my-4">
          {this.props.network?.experimental && (
            <AlertMessage variant="info" dismissible={false}>
              This network was added to REStake automatically and has not been
              thoroughly tested yet.{" "}
              <a
                href="https://github.com/eco-stake/restake/issues"
                target="_blank"
              >
                Raise an issue
              </a>{" "}
          ...
          {!this.state.providerError === "keplr" && (
            <AlertMessage
              variant="warning"
              dismissible={true}
              onClose={() => this.setState({ providerError: false })}
            >
              Please install the{" "}
              <a
          ...
          <p className="mt-5 text-center">
            ...
            <em>Delegate</em> transactions on your behalf for 1 year{" "}
            <a
            ...
            .<br />
            They will only be authorized to delegate to their own validator. You
        ...
        <footer className="d-flex flex-wrap justify-content-between align-items-center py-3 my-4 border-top">
          ...
          <div className="col-md-4 align-items-center text-center me-lg-auto">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-reset text-decoration-none d-block mb-2"
            >
              <span className="d-none d-sm-inline">Built with 💚&nbsp;</span> by
              ECO Stake 🌱
            </a>
            <a
              href={`https://${this.props.directory.domain}`}
              target="_blank"
              className="text-reset text-decoration-none d-block small"
            >
              <span className="d-none d-sm-inline">Interchain APIs from</span>{" "}
              <u>cosmos.directory</u>
            </a>
          </div>
        ...
        <AddressModal
          ...
          onHide={() => this.setState({ showAddressModal: false })}
          network={this.props.network}
          networks={Object.values(this.props.networks)}
          address={this.state.address}
          wallet={this.state.wallet}
          ...
          setAddress={(value) =>
            this.setState({ address: value, showAddressModal: false })
          }
        />
        {this.props.network && (
          <SendModal
