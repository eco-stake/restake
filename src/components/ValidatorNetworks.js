...
function ValidatorNetworks(props) {
  ...
  const validatorNetworks = _.uniqBy(
    _.compact(
      registryData.chains.map((chain) => {
          ...
          return {
            ...
            image: (
              <NetworkImage
                ...
                height={20}
                width={20}
                className={active ? "border border-2 border-info" : ""}
              />
            ),
      ...
      {validatorNetworks.map((validatorNetwork) => {
        return (
          <OverlayTrigger
            ...
            overlay={
              <Tooltip id={`tooltip-${validatorNetwork.key}`}>
                {validatorNetwork.name}
              </Tooltip>
            }
          >
            <Link
              to={validatorNetwork.path}
              onClick={() =>
                !validatorNetwork.active &&
                setNetworkLoading(validatorNetwork.path)
              }
            >
