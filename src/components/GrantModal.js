...
function GrantModal(props) {
  ...
  function messageType() {
    return state.messageTypeValue === "Custom"
      ? state.customMessageTypeValue
      : state.messageTypeValue;
  }
  ...
  function grantee() {
    return state.granteeValue === "custom"
      ? state.customGranteeValue
      : state.granteeValue;
  }
  ...
  function favourites() {
    return props.favouriteAddresses.filter(
      (el) => el.address !== props.address,
    );
  }
          ...
          <Form onSubmit={handleSubmit}>
            <Form.Group className="mb-3">
              ...
              {state.granteeValue === "custom" && (
                <Form.Control
                  placeholder={`${network.prefix}1...`}
                  className="mt-1"
                  type="text"
                  name="customGranteeValue"
                  required={true}
                  value={state.customGranteeValue}
                  isInvalid={!validGrantee()}
                  onChange={handleInputChange}
                />
              )}
            </Form.Group>
            ...
            <Form.Group className="mb-3">
              ...
              <select
                ...
                value={state.grantTypeValue}
                onChange={handleInputChange}
              >
                <option value="/cosmos.authz.v1beta1.GenericAuthorization">
              ...
              <>
                    ...
                    {messageTypes.map((type) => {
                      return (
                        <option key={type} value={type}>
                          {_.startCase(
                            type.split(".").slice(-1)[0].replace("Msg", ""),
                          )}
                        </option>
                      );
                    })}
                ...
                {state.messageTypeValue === "Custom" && (
                  <Form.Group className="mb-3">
                    ...
                    <Form.Control
                      type="text"
                      name="customMessageTypeValue"
                      required={true}
                      value={state.customMessageTypeValue}
                      onChange={handleInputChange}
                    />
                  </Form.Group>
                )}
            ...
            <p>
              <strong>
                Incorrect use of Authz grants can be as dangerous as giving away
                your mnemonic.
              </strong>{" "}
              Make sure you trust the Grantee address and understand the
            ...
            <Collapse in={showLedger}>
              <pre className="text-wrap">
                <code>
                  <p>
                    {daemon_name ? daemon_name : <kbd>{"<chaind>"}</kbd>} tx
                    authz grant \<br />
                    <kbd>{grantee() || "<grantee>"}</kbd> generic \<br />
                    --msg-type <kbd>{messageType()}</kbd> \<br />
                    --expiration{" "}
                    <kbd>{moment(state.expiryDateValue).unix()}</kbd> \<br />
                    --chain-id {chain_id} \<br />
                    --node https://rpc.cosmos.directory:443/{network.name} \
                    <br />
