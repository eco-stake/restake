...
function SendModal(props) {
  ...
  function handleSubmit(event) {
    ...
    const messages = [buildSendMsg(address, recipient(), [coinValue])];
    console.log(messages);

    props.signingClient
      .signAndBroadcast(wallet.address, messages, null, state.memoValue)
      .then(
        (result) => {
  ...
  function buildSendMsg(address, recipient, amount) {
    let message = buildExecableMessage(
      MsgSend,
  ...
  async function setAvailableAmount() {
    ...
    props.signingClient.simulate(wallet.address, [message]).then(
      (gas) => {
        const gasPrice = props.signingClient.getFee(gas).amount[0].amount;
        const amount = divide(
          subtract(bignumber(props.balance.amount), gasPrice),
          decimals,
        );
  ...
  function recipient() {
    return state.recipientValue === "custom"
      ? state.customRecipientValue
  ...
  function valid() {
    ...
    return (
      validRecipient() && coinAmount() && wallet?.hasPermission(address, "Send")
    );
  }
            ...
            <Form.Group className="mb-3">
              ...
              {state.recipientValue === "custom" && (
                <Form.Control
                  ...
                  className="mt-1"
                  type="text"
                  name="customRecipientValue"
                  required={true}
                  value={state.customRecipientValue}
