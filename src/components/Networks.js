...
function Networks(props) {
  ...
  function filteredNetworks(networks, filter) {
    ...
    switch (group) {
      case "favourites":
        searchResults = searchResults.filter((network) =>
          favourites.includes(network.path),
        );
        break;
    }
    ...
    const searcher = new FuzzySearch(searchResults, ["prettyName"], {
      sort: true,
    });
    ...
    return (
      <Row xs={1} sm={2} lg={3} xxl={4} className="g-4 justify-content-center">
        {networks.map((network) => (
          <Col
            key={network.path}
            className={network.online ? "" : "opacity-50"}
          >
            <Card>
              <span
                onClick={() => toggleFavourite(network)}
                role="button"
                className="text-right position-absolute top-0 end-0 py-1 px-2"
                style={{ zIndex: 2 }}
              >
                {renderFavourite(network)}
              </span>
              <span
                role={network.online ? "button" : ""}
                ...
                <Row className="g-0">
                  <Col xs={3} className="text-center">
                    <NetworkImage
                      ...
                      width={60}
                      height={60}
                      className="m-2 shadow overflow-hidden rounded-circle"
                    />
                  </Col>
                  <Col xs={9}>
                    <Card.Body>
    ...
    <>
      <div className="d-flex flex-nowrap justify-content-center align-items-start mb-3 position-relative">
          ...
          >
            <Nav.Item>
              <Nav.Link
                eventKey="favourites"
                disabled={
                  filteredNetworks(networks, { ...filter, group: "favourites" })
                    .length < 1
                }
              >
                Favourites
        ...
        <div className="d-flex d-lg-none justify-content-end">
          <select
            className="form-select w-auto h-auto d-lg-none"
        ...
        <div className="flex-fill d-none d-lg-flex justify-content-end"></div>
      </div>
