...
function ValidatorGrants(props) {
  ...
  const { stakeGrant, maxTokens, validators, grantsValid, grantsExist } =
    grants || {};
  ...
  useEffect(() => {
    setState({
      ...
      maxTokensValue:
        maxTokens && state.maxTokensValue === ""
          ? divide(bignumber(maxTokens), pow(10, network.decimals))
          : maxTokens
          ? state.maxTokensValue
          : "",
    });
  }, [operator]);
  ...
  function expiryDate() {
    const stakeExpiry =
      stakeGrant && stakeGrant.expiration && moment(stakeGrant.expiration);
    return stakeExpiry;
  }
  ...
  function handleSubmit(event) {
    ...
    props.signingClient.signAndBroadcast(wallet.address, messages).then(
      (result) => {
  ...
  function grantInformation() {
    return (
      <>
        <p className="small">
          {operator.moniker} will be able to carry out the following
          transactions on your behalf.
        </p>
        <p className="small">
          <strong>Delegate</strong> - allowed to delegate{" "}
          <em>
            {maxTokensDenom() ? (
              <Coins
                coins={{ amount: maxTokensDenom(), denom: network.denom }}
          ...
          <em>
            {!state.validators
              ...
              : !state.validators.length ||
                (state.validators.length === 1 &&
                  state.validators.includes(operator.address))
              ? "only their own validator"
              : "validators " + state.validators.join(", ")}
          </em>
          .
        </p>
        <p className="small">
          This grant will expire automatically on{" "}
          <em>{state.expiryDateValue}</em>.
        </p>
        <p className="small">
          REStake only re-delegates {operator.moniker}'s accrued rewards and
          tries not to touch your balance.
        </p>
        <p className="small">
          <em>
            REStake previously required a Withdraw grant but this is no longer
            necessary.
          </em>
        </p>
      </>
  ...
  return (
    <>
      ...
      {props.restakePossible && !props.delegation && (
        <AlertMessage variant="warning" dismissible={false}>
          You must delegate to {operator.moniker} before they can REStake for
          you.
        </AlertMessage>
      )}
      {error && (
      ...
      <Table>
        <tbody className="table-sm small">
          ...
          <tr>
            ...
            <td>
              <Coins
                coins={{ amount: props.rewards, denom: network.denom }}
                asset={network.baseAsset}
                fullPrecision={true}
              />
            </td>
          ...
          {state.maxTokens && (
            <tr>
              ...
              <td
                className={
                  !props.rewards || larger(state.maxTokens, props.rewards)
                    ? "text-success"
                    : "text-danger"
                }
              >
                <Coins
                  coins={{ amount: state.maxTokens, denom: network.denom }}
                  asset={network.baseAsset}
                  fullPrecision={true}
                />
              </td>
      ...
      {grantsExist && !props.restakePossible && <>{grantInformation()}</>}
              ...
              <fieldset disabled={!props.address || !props.wallet}>
                  ...
                  <div className="mb-3">
                    <div className="input-group">
                      <Form.Control
                        type="number"
                        name="maxTokensValue"
                        min={divide(1, pow(10, network.decimals))}
                        className={
                          !maxTokensValid() ? "is-invalid" : "is-valid"
                        }
                        step={step()}
                        placeholder={
                          maxTokens
                            ? divide(
                                bignumber(maxTokens),
                                pow(10, network.decimals),
                              )
                            : "Unlimited"
                        }
                        required={false}
                        value={state.maxTokensValue}
                        onChange={handleInputChange}
                      />
                      <span className="input-group-text">{network.symbol}</span>
                    </div>
                    <div className="form-text text-end">
                      Reduces with every delegation made by the validator
                      <br />
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    required={true}
                    value={state.expiryDateValue}
                    onChange={handleInputChange}
                  />
                  <div className="form-text text-end">
                    Date the grant will expire. After this date you will need to
                    re-grant
                  </div>
                </Form.Group>
