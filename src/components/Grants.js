...
import { Spinner, Table, Nav, Button } from "react-bootstrap";
...
function Grants(props) {
  ...
  const [grantsLoading, setGrantLoading] = useReducer(
    (grantsLoading, newGrantsLoading) => ({
      ...grantsLoading,
      ...newGrantsLoading,
    }),
    {},
  ...
  function filteredGrants(grants, filter) {
    ...
    const searcher = new FuzzySearch(
      searchResults,
      ["grantee", "granter", "authorization.@type", "authorization.msg"],
      { sort: true },
    );
  ...
  function renderGrant(grant) {
    ...
    const validator =
      filter.group === "granter" && validatorForGrantAddress(grant.grantee);
    const favourite =
      props.favouriteAddresses &&
      props.favouriteAddresses.find(
        (el) => el.address === (filter.group === "granter" ? grantee : granter),
      );
        ...
        <td className="text-break">
          {filter.group === "grantee" ? (
            <div className="d-flex">
              <Favourite
                favourites={props.favouriteAddresses}
                value={granter}
            ...
            favourite?.label || grantee
          )}
        </td>
        <td>{authorization["@type"].split(".").slice(-1)[0]}</td>
        <td className="d-none d-lg-table-cell">{renderGrantData(grant)}</td>
        <td className="d-none d-md-table-cell">
          <Moment format="LLL">{expiration}</Moment>
        </td>
    ...
    <>
      {!props.grantQuerySupport && (
        <AlertMessage variant="warning">
          This network doesn't fully support this feature just yet.{" "}
          <span
            role="button"
            className="text-decoration-underline"
            onClick={props.showFavouriteAddresses}
          >
            Save addresses
          </span>{" "}
          to see them here.
      ...
      {props.grantQuerySupport && isNanoLedger && (
        <AlertMessage variant="warning" dismissible={false}>
          <p>
            Ledger devices can't send Authz transactions just yet. Full support
            will be enabled as soon as it is possible.
          </p>
          <p className="mb-0">
            <span
            ...
            >
              <Nav.Item>
                <Nav.Link
                  eventKey="granter"
                  disabled={
                    filteredGrants(grants, { ...filter, group: "granter" })
                      .length < 1
                  }
                >
                  Granted by me
              ...
              <Nav.Item>
                <Nav.Link
                  eventKey="grantee"
                  disabled={
                    filteredGrants(grants, { ...filter, group: "grantee" })
                      .length < 1
                  }
                >
                  Granted to me
          ...
          <Table className="align-middle table-striped">
            <thead>
              <tr>
                ...
                {filter.group === "granter" && <th></th>}
              </tr>
            </thead>
            <tbody>{results.map((item) => renderGrant(item))}</tbody>
          </Table>
        )}
        {results.length < 1 && (
          <p className="text-center my-5">
            <em>No grants found</em>
          </p>
        )}
      </div>
      <GrantModal
