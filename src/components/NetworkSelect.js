      ...
      Network(data).then((network) => {
        ...
        if (network.connected) {
          network
            ...
            .catch((error) => {
              setError(
                "Unable to connect to this network currently. Try again later."
              );
              setLoading(false);
            });
        } else {
          ...
          {selectedNetwork && (
            <Form onSubmit={handleSubmit}>
                        ...
                        <div className="row">
                          ...
                          <div className="col text-end pt-1">
                            {network.operators.length > 0 && (
                              <small>
                                {network.operators.length} Operator
                                {network.operators.length > 1 ? "s" : ""}
                              </small>
                            )}
                            {network.authz ? (
                              <Badge className="ms-3 rounded-pill" bg="success">
                                Authz
                              </Badge>
                            ) : (
                              <Badge
                                className="ms-3 rounded-pill text-decoration-line-through"
                                bg="danger"
                              >
                                Authz
                              </Badge>
              ...
              {!error && !selectedNetwork.authzSupport && (
                <p>
                  <em>
                    This network does not support Authz yet. You can manually
                    stake and compound for now
                  </em>
                </p>
              )}
              {!error &&
                selectedNetwork.authzSupport &&
                selectedNetwork.getOperators(validators).length < 1 && (
                  <p>
                    <em>
                      This network supports Authz but there are no operators
                      just yet. You can manually REStake for now
                    </em>
                  </p>
                )}
              {!loading ? (
