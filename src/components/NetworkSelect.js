...
function NetworkSelect(props) {
  ...
  useEffect(() => {
    const networks = Object.values(props.networks).sort((a, b) =>
      a.name > b.name ? 1 : -1,
    );
    ...
    const isFavourite =
      selectedNetwork &&
      props.favourites &&
      props.favourites.includes(selectedNetwork.path);
    setOptions({
      networks: networkOptions,
      network: isFavourite
        ? `favourite-${selectedNetwork.path}`
        : selectedNetwork && selectedNetwork.path,
      grouped: props.favourites &&
        props.favourites.length > 0 && [
          {
            label: "Favourites",
            options: networkOptions
              .filter((el) => props.favourites.includes(el.value))
              .map((el) => {
  ...
  return (
    <>
      <Button
        onClick={handleOpen}
        variant="link"
        className="d-flex flex-nowrap text-nowrap align-items-center text-reset text-decoration-none border-secondary btn-outline-light"
        role="button"
      >
        {props.network ? (
          <>
            ...
            <div className="d-none d-md-block ms-2">
              {props.network.authzSupport ? (
                ...
                <Badge
                  className="rounded-pill text-decoration-line-through"
                  bg="danger"
                >
                  Authz
                </Badge>
                ...
                <div className="col">
                  <Select
                    ...
                    formatOptionLabel={(network) => (
                      <div
                        className={
                          "d-flex" + (!network.online ? " text-muted" : "")
                        }
                      >
                        <div className="pe-2">
                          <NetworkImage
                        ...
                        <div className="pt-1 me-auto">
                          <span className="ms-1">
                            {network.label}{" "}
                            {!network.online && <small>(Offline)</small>}
                          </span>
                        </div>
                        <div className="text-end pt-1 d-none d-sm-block">
                          {network.operatorCount > 0 && (
                            <small>
                              {network.operatorCount} Operator
                              {network.operatorCount > 1 ? "s" : ""}
                            </small>
                          )}
                        </div>
                        <div className="text-end pt-1">
                          {network.authz ? (
                            ...
                            <Badge
                              className={
                                `ms-3 rounded-pill text-decoration-line-through` +
                                (!network.online ? " opacity-50" : "")
                              }
                              bg="danger"
                            >
