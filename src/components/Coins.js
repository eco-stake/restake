...
function Coins(props) {
  const { asset, coins, fullPrecision, inBaseDenom, hideValue, className } =
    props;
  ...
  function amount(coins) {
    ...
    return round(
      divide(bignumber(coins.amount), Math.pow(10, decimals)),
      prec,
    ).toLocaleString(undefined, { maximumFractionDigits: prec });
  }

  function value(coins) {
    return (
      (coins.amount / Math.pow(10, decimals)) *
      coingecko.usd
