...
function Validators(props) {
  ...
  const showCommission =
    results &&
    Object.values(results).find((el) => el.isValidatorOperator(address));
  ...
  function sortValidators(validators) {
    ...
    return _.sortBy(validators, ({ operator_address: address }) => {
      ...
      const delegation = delegations && delegations[address];
      const operator =
        operators && operators.find((el) => el.address === address);
  ...
  function filteredValidators(validators, filter) {
    ...
    const searcher = new FuzzySearch(searchResults, ["description.moniker"], {
      sort: true,
    });
  ...
  function renderValidator(validator) {
    ...
    const denomRewards =
      rewards &&
      rewards.reward.find((reward) => reward.denom === network.denom);
    ...
    return (
      <tr key={validatorAddress} className={rowVariant}>
        ...
        <td width={30}>
          <ValidatorImage validator={validator} width={30} height={30} />
        </td>
        <td>
          <span
            role="button"
        ...
        <td className="d-none d-sm-table-cell text-center">
          {!props.isLoading ? (
            <ManageRestake
              ...
              openGrants={() =>
                props.showValidator(validator, { activeTab: "restake" })
              }
            />
          ) : (
            <Spinner
          ...
          <td className="text-center">
            {Object.keys(props.validatorApy).length > 0 ? (
              props.validatorApy[validatorAddress] !== undefined ? (
                <small>
                  {Math.round(
                    props.validatorApy[validatorAddress] * 100,
                  ).toLocaleString() + "%"}
                </small>
              ) : (
                "-"
        ...
        <td className="d-none d-lg-table-cell text-center">
          <small>
            {format(validator.commission.commission_rates.rate * 100, 2)}%
          </small>
        </td>
        <td className="d-none d-sm-table-cell">
          {delegations ? (
            <small>
              <Coins coins={delegationBalance} asset={network.baseAsset} />
            </small>
          ) : (
            address && (
              <Spinner
          ...
          <div className="d-grid flex-row gap-2 gap-sm-3 d-flex justify-content-end align-items-center">
            ...
            {props.manageButton ? (
              <Button
                size="sm"
                onClick={() =>
                  props.showValidator(validator, { activeTab: "delegate" })
                }
              >
                {props.manageButton}
              </Button>
              ...
              delegation ? (
                <Dropdown>
                  ...
                  <Dropdown.Menu>
                    {operator && props.restakePossible && (
                      <>
                        <Dropdown.Item
                          as="button"
                          disabled={
                            !wallet?.hasPermission(address, "Grant") ||
                            !wallet?.hasPermission(address, "Revoke")
                          }
                          onClick={() =>
                            props.showValidator(validator, {
                              activeTab: "restake",
                            })
                          }
                        >
                    ...
                    <ClaimRewards
                      ...
                      address={address}
                      wallet={wallet}
                      validatorRewards={props.validatorRewards([
                        validatorAddress,
                      ])}
                      signingClient={props.signingClient}
                    ...
                    <Dropdown.Item
                      as="button"
                      disabled={!wallet?.hasPermission(address, "Delegate")}
                      onClick={() =>
                        props.showValidator(validator, {
                          activeTab: "delegate",
                        })
                    ...
                    <Dropdown.Item
                      as="button"
                      disabled={
                        !wallet?.hasPermission(address, "BeginRedelegate")
                      }
                      onClick={() =>
                        props.showValidator(validator, { redelegate: true })
                      }
                    >
                      Redelegate
                    </Dropdown.Item>
                    <Dropdown.Item
                      as="button"
                      disabled={!wallet?.hasPermission(address, "Undelegate")}
                      onClick={() =>
                        props.showValidator(validator, { undelegate: true })
                      }
                    >
                      Undelegate
                    </Dropdown.Item>
                ...
                wallet?.hasPermission(address, "Delegate") ? (
                operator ? (
                  <OverlayTrigger
                    placement="top"
                    rootClose={true}
                ...
                <Button
                  variant="primary"
                  size="sm"
                  onClick={() => props.showValidator(validator)}
                >
                  View
                </Button>
              )
  ...
  return (
    <>
      <div className="d-flex flex-wrap justify-content-center align-items-start mb-3 position-relative">
        <div className="d-none d-md-flex me-5">
          <div className="input-group">
            ...
            <span className="input-group-append">
              <button
                className="btn btn-light text-dark border-left-0 border"
                type="button"
                onClick={() => setFilter({ ...filter, keywords: "" })}
              >
                <XCircle />
              </button>
            </span>
        ...
        <div
          className={`${
            !props.modal && "d-lg-flex"
          } d-none position-absolute mx-auto justify-content-center align-self-center`}
        >
          <Nav
            fill
            variant="pills"
            activeKey={filter.group}
            className={`flex-row${props.modal ? " small" : ""}`}
            onSelect={(e) => setFilter({ ...filter, group: e })}
          >
            <Nav.Item>
              <Nav.Link
                eventKey="delegated"
        ...
        <div
          className={`d-flex ${
            !props.modal && "d-lg-none"
          } justify-content-center`}
        >
          <select
            className="form-select w-auto h-auto"
      ...
      {results.length > 0 && (
        <Table className="align-middle table-striped">
            ...
            <tr>
              ...
              {network.apyEnabled && (
                <th className="text-center">
                  <TooltipIcon
                    icon={
                      <span className="text-decoration-underline">APY</span>
                    }
                    identifier="delegations-apy"
                  >
                    <div className="mt-2 text-center">
                      <p>
                        Based on commission, compounding frequency and estimated
                        block times.
                      </p>
                      <p>
                        This is a best case scenario and may not be 100%
                        accurate.
                      </p>
                    </div>
                  </TooltipIcon>
          ...
          <tbody>{results.map((item) => renderValidator(item))}</tbody>
          <tfoot>
            <tr>
              ...
              {network.apyEnabled && <td className="text-center"></td>}
                  ...
                  <Coins
                    coins={{
                      amount: results.reduce((sum, result) => {
                        const delegation =
                          delegations && delegations[result.operator_address];
                      ...
                      <Coins
                        coins={{
                          amount: results.reduce((sum, result) => {
                            ...
                            ]?.reward?.find((el) => el.denom === network.denom);
