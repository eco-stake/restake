...
function Validators(props) {
  ...
  function filteredOperators() {
    const results = _.pick(
      props.validators,
      props.operators.map((el) => el.address)
    );
  ...
  function calculateApy(validatorCommission, periodPerYear) {
    const chainApr =
      (1 + props.inflation / props.blocksPerYear) ** props.blocksPerYear - 1;
    const realApr = chainApr * (1 - validatorCommission);
        ...
        <td className="text-center">
          {isOperator ? (
            <TooltipIcon
              ...
              identifier={item.operator_address}
      ...
      {(filteredOperators().length > 0 || filteredValidators().length > 0) && (
        <Table className="align-middle">
          <tbody>
            ...
            {filteredOperators().map(([validator_address, item], i) =>
              renderItem(item, true)
            )}
            {filteredValidators().map(([validator_address, item], i) =>
              renderItem(item)
            )}
          </tbody>
        </Table>
