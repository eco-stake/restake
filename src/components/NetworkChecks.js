...
function NetworkChecks(props) {
  ...
  function renderCheck({
    ...
    const content = (
      <div className="small">
        {icon ? (
          ...
          <InfoCircle className="me-2 mb-1" />
        )}
    ...
    return (
      <li
        key={identifier}
        className={`list-group-item list-group-item-${className}`}
      >
        <TooltipIcon
          icon={content}
  ...
  const operatorCheck = {
    title: (
      <span>
        <strong>{network.operatorCount}</strong> REStake operators
      </span>
    ),
    failTitle: "No REStake operators",
    failDescription:
      "There are no operators for this network yet. You can stake and compound manually in the meantime.",
  };
  if (!network.authzSupport && network.operatorCount > 0) {
    (operatorCheck.failTitle = operatorCheck.title),
      (operatorCheck.failDescription =
        "Authz is disabled but there are operators ready when support is added.");
  }
  ...
  const testedCheck = {
    ...
    failDescription:
      "This network was added to REStake automatically and has not been thoroughly tested yet.",
  };
  if (!network.authzSupport && network.operatorCount > 0) {
    (testedCheck.failTitle = testedCheck.title),
      (testedCheck.failDescription =
        "Authz is disabled but all other features have been fully tested.");
  }
    ...
    <ul className={className} style={props.style}>
      {[
        ...
        renderCheck({
          title: (
            <strong>{`${Math.round(
              network.estimatedApr * 100,
            ).toLocaleString()}% APR`}</strong>
          ),
        ...
        renderCheck({
          ...
          failDescription:
            "This network doesn't support Authz just yet. You can stake and compound manually, REStake will update automatically when support is added.",
