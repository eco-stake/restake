...
function ValidatorProfile(props) {
  ...
  const bondedTokens = () => {
    ...
    return (
      <Coins
        coins={{ amount: amount, denom: network.denom }}
  ...
  const uptime = () => {
    ...
    return (
      <span className="p-0">
        {_.round(period.uptime * 100, 2)}% (missed{" "}
        {blockPeriod.missed.toLocaleString()} of{" "}
        {blockPeriod.blocks.toLocaleString()} blocks)
      </span>
    );
  };
        ...
        <tbody className="table-sm small">
          <tr>
            ...
            <td className="text-break">
              <span className="p-0">{validator.operator_address}</span>
            </td>
          </tr>
          {!active() && (
            <tr>
          ...
          <tr>
            ...
            <td>
              {!!operator ? (
                ...
                <TooltipIcon
                  icon={<XCircle className="opacity-50 p-0" />}
          ...
          {network.apyEnabled && (
            <tr>
              <td scope="row">
                <TooltipIcon
                  icon={
                    <span className="p-0 text-decoration-underline">APY</span>
                  }
              ...
              <td>
                {Object.keys(props.validatorApy).length > 0 ? (
                  props.validatorApy[validator.operator_address] ? (
                    <span className="p-0">
                      {Math.round(
                        props.validatorApy[validator.operator_address] * 100,
                      ).toLocaleString()}
                      %
                    </span>
                  ) : (
          ...
          <tr>
            ...
            <td>
              <span className="p-0">{bondedTokens()}</span>
            </td>
          </tr>
          <tr>
            <td scope="row">Rank</td>
            <td>
              <span className="p-0">#{validator.rank}</span>
            </td>
          </tr>
          {!!securityContact() && (
            <tr>
              ...
              <td>
                <a className="p-0" href={`mailto:${securityContact()}`}>
                  {securityContact()}
                </a>
              </td>
            </tr>
