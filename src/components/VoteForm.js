...
function VoteForm(props) {
  ...
  function handleSubmit(event) {
    ...
    props.signingClient.signAndBroadcast(wallet.address, [message]).then(
      (result) => {
  ...
  function buttonText() {
    if (!proposal.isVoting)
      return proposal.isDeposit ? "Voting not started" : "Voting ended";
      ...
      <Form onSubmit={handleSubmit}>
        <Form.Group className="border-top pt-2 my-2">
          <div className="row pe-lg-5">
            {_.chunk(Object.entries(choices), 2)
              .map((group, index) => {
                return (
