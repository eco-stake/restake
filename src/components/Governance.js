...
function Governance(props) {
  ...
  async function getProposals(opts) {
    ...
    try {
      ...
      setTallies(
        newProposals.reduce((sum, proposal) => {
          if (
            !_.every(
              Object.values(proposal.final_tally_result),
              (el) => el === "0",
            )
          ) {
  ...
  async function getVotes(proposals, opts) {
    ...
    const calls = proposals
      .filter((el) => el.status === "PROPOSAL_STATUS_VOTING_PERIOD")
      .map((proposal) => {
        return () => {
          ...
          return props.queryClient
            .getProposalVote(proposal_id, address)
            .then((result) => {
